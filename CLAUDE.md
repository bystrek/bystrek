...
`bystrek.dev` now serves `ui` (SvelteKit: subscribe UI, service worker) and `api.bystrek.dev` serves `api` (NestJS + Drizzle + Bun: Postgres, push subscribe/send) — both real services, verified with a real push notification through the deployed stack. Chat, auth, and the data model beyond push subscriptions are still not built; those are deferred, not next-session-immediate. `docker-compose.yml`/`Caddyfile` (`~/bystrek/`) and Dockge's own compose file (`~/dockge/docker-compose.yml`) are still managed by hand on the droplet and scp'd down to `infra/` as a reference copy only. Deploy itself is no longer manual: GitHub Actions (`.github/workflows/deploy.yml`) builds `api`/`ui` to GHCR, tests pass, then a single `deploy` job — gated by a GitHub Environment (required reviewer) — SSHes in with a key restricted via a forced command (`authorized_keys`, can only run `infra/deploy.sh`, nothing else) to `docker compose pull && up -d --remove-orphans`. Watchtower has been removed entirely (repo + droplet) now that this path is proven; don't reintroduce it without revisiting `devlog/2026-08-09-day-05.md`'s reasoning. Dockge stays for visibility and manual overrides.
