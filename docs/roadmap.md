...
**Calendar CRUD via chat.** `api/src/calendar/`: `calendar_credentials` table (one CalDAV account per user, `password` tier-2 encrypted via the existing `field-encryption` wrapper), a `tsdav`-backed `CalendarService` talking to Infomaniak kCalendar (list/create/update/delete), and `ical.js` for building/parsing RRULE-bearing event bodies — recurrence expansion itself is the CalDAV server's job, not client-side (see devlog day 12). Four tools (`list_calendar_events`/`create_calendar_event`/`update_calendar_event`/`delete_calendar_event`) registered into the chat tool-call loop; `CHAT_TOOLS` is now assembled from per-domain tool providers (calendar is the first) rather than a static empty array, and `ChatTool.handler` gained a `userId` parameter so a tool can scope its work to the requesting user. `ui`'s profile page gained a Calendar settings section (CalDAV URL/username/app-specific password/calendar name) — verified live in a browser (save, reload-persists, disconnect). Full CRUD verified against the real Infomaniak account: read (19 real events fetched and parsed, 0 failures) and a live create → update → delete round-trip against a throwaway test event, all cleaned up after. Note: the CalDAV **username** Infomaniak expects is the short login ID from `config.infomaniak.com`'s sync assistant (e.g. `AB12345`), not the account email — worth calling out on the profile page's calendar form since it's not obvious. System prompt carries the current date/time and a fixed `Europe/Warsaw` timezone (see devlog day 12); working-hours context is still missing — a later refinement, not blocking. Mutating tools (`propose_create_calendar_event`/`propose_update_calendar_event`/`propose_delete_calendar_event`) stage the change and require a `confirm_calendar_action` call in a later chat turn to execute, per `docs/architecture.md`'s write-safety requirement — `list_calendar_events` stays direct (read-only).
